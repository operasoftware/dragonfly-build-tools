...
def create_manifests(path, domain_token=None, tag=""):
    web_path = path
    if domain_token:
        pos = web_path.find(domain_token)
        if pos > -1:
            web_path = web_path[pos + len(domain_token):]
    web_path = "/".join(map(lambda p: quote(p), web_path.split(os.path.sep)))
    if not os.path.exists(os.path.join(path, MANIFEST_DIR)):
        os.makedirs(os.path.join(path, MANIFEST_DIR))
    for name in os.listdir(path):
         ...
         if match:
            ...
            add_manifest(path, web_path, name, manifest)
...
def main(argv=sys.argv):
    ...
    create_manifests(path, domain_token=options.domain, tag=options.tag)
